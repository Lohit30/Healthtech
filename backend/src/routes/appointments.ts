...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function isPatient(req: Request) { return req.user?.role === 'patient'; }
function isAdminOrDoctor(req: Request) {
    return req.user?.role === 'admin' || req.user?.role === 'doctor';
}

/** Mark an availability slot as booked (1) or free (0). No-op if id is null. */
function setSlotBooked(slotId: number | null | undefined, booked: 0 | 1) {
    if (!slotId) return;
    db.prepare('UPDATE doctor_availability SET is_booked = ? WHERE id = ?').run(booked, slotId);
}
...
router.get('/:id', (req: Request, res: Response) => {
  ...
  `).get(req.params.id) as { user_id: number | null } | undefined;
    ...
    if (isPatient(req) && (appt as any).user_id !== req.user!.id) {
...
router.post('/', (req: Request, res: Response) => {
    ...
    if (availability_id) {
        const slot = db.prepare('SELECT * FROM doctor_availability WHERE id = ?').get(availability_id) as
            { id: number; is_booked: number } | undefined;
        if (!slot) return res.status(404).json({ error: 'Availability slot not found' });
        if (slot.is_booked) return res.status(409).json({ error: 'That slot is already booked' });
    }

    const result = db.prepare(
        `INSERT INTO appointments (patient_id, user_id, doctor_id, availability_id, date, status)
         VALUES (?, ?, ?, ?, ?, ?)`
    ).run(patient_id_from_body, req.user!.id, doctor_id, availability_id ?? null, date, status || 'scheduled');
...
router.put('/:id', (req: Request, res: Response) => {
    const existing = db.prepare('SELECT * FROM appointments WHERE id = ?').get(req.params.id) as
        { user_id: number | null; patient_id: number | null; doctor_id: number; availability_id: number | null; date: string; status: string } | undefined;
    ...
    if (isPatient(req)) {
        ...
        if (availability_id !== undefined && availability_id !== existing.availability_id) {
            ...
            if (availability_id) {
                const newSlot = db.prepare('SELECT * FROM doctor_availability WHERE id = ?').get(availability_id) as
                    { is_booked: number } | undefined;
                if (!newSlot) return res.status(404).json({ error: 'New slot not found' });
                if (newSlot.is_booked) return res.status(409).json({ error: 'New slot is already booked' });
                setSlotBooked(availability_id, 1);
            }
        }

        db.prepare('UPDATE appointments SET date = ?, availability_id = ? WHERE id = ?')
            .run(date, availability_id ?? existing.availability_id, req.params.id);
        return res.json(db.prepare('SELECT * FROM appointments WHERE id = ?').get(req.params.id));
    }
    ...
    db.prepare(
        `UPDATE appointments SET patient_id=?, doctor_id=?, availability_id=?, date=?, status=? WHERE id=?`
    ).run(
...
router.delete('/:id', (req: Request, res: Response) => {
    const existing = db.prepare('SELECT * FROM appointments WHERE id = ?').get(req.params.id) as
        { user_id: number | null; availability_id: number | null } | undefined;
