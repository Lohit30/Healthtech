...
router.get('/:patientId', authenticate, (req: Request, res: Response) => {
    ...
    // Fetch Vitals
    let vitals = db.prepare('SELECT * FROM patient_vitals WHERE patient_id = ? ORDER BY created_at DESC LIMIT 1').get(patientId) as any;
    if (!vitals) {
        // Fallback to realistic samples if vitals table is empty for this patient
        const bases: Record<string, { hr: number; spo2: number; glucose: number }> = {
    ...
    const recentNote = db.prepare('SELECT * FROM consultation_notes WHERE patient_id = ? ORDER BY created_at DESC LIMIT 1').get(patientId) as any;
    ...
    if (prescriptions.length === 0) {
        ...
        prescriptions.forEach((rx, index) => {
            if (index % 2 === 0) doc.rect(50, rowY, 510, 20).fill('#F8FAFC');
            doc.fill('#0F172A');
    ...
    doc.font('Helvetica-Oblique').fontSize(8).fillColor('#94A3B8').text(
        'This is a digitally generated report from RuralCare Healthcare Management. Not valid for medico-legal purposes without authorized signature.',
        50, 700, { align: 'center' }
    );
