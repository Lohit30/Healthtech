...
// Seed pharmacy user
const pharmCount = db.prepare('SELECT COUNT(*) as count FROM users WHERE role = ?').get('pharmacy').count;
if (pharmCount === 0) {
    const hashedPassword = bcryptjs_1.default.hashSync('Pharmacy@1234', 10);
    db.prepare('INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, ?)').run('Village Pharmacy', 'pharmacy@ruralcare.com', hashedPassword, 'pharmacy');
